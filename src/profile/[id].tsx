...
function OrderCard({ order, buildName, parts }) {
  const [expanded, setExpanded] = useState(false);

  const statusColor = {
    paid: "#0FD980", fulfilled: "#78A3FF",
    pending: "#FFD66B", cancelled: "#FF6B6B"
  }[order.status] || T.textDim;

  return (
    <div style={{
      borderRadius: 10, border: `1px solid ${expanded ? "rgba(15,217,128,0.25)" : T.border}`,
      background: "rgba(255,255,255,0.02)", marginBottom: 12, overflow: "hidden",
      ...
      >
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: T.text, marginBottom: 4 }}>{buildName}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 16, fontWeight: 800, color: T.green }}>${order.total_price}</span>
          <span style={{
            fontSize: 10, letterSpacing: 1.5, fontWeight: 700, padding: "3px 10px",
            borderRadius: 20, background: `${statusColor}18`,
      ...
      {/* Expanded parts */}
      {expanded && (
        <div style={{ borderTop: `1px solid ${T.border}`, padding: "16px 20px", background: "rgba(0,0,0,0.2)" }}>
          {parts ? (
            <>
              <div style={{ fontSize: 9, letterSpacing: 3, color: T.textDim, fontFamily: T.mono, marginBottom: 12 }}>PARTS ORDERED</div>
              {Object.entries(parts).map(([cat, part]) => (
                <div key={cat} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${T.border}` }}>
                  <div>
                    <div style={{ fontSize: 9, color: T.textDim, letterSpacing: 1, fontFamily: T.mono }}>{cat}</div>
            ...
            <div style={{ fontSize: 12, color: T.textDim, fontFamily: T.mono }}>
              Parts detail not available for this order.
            </div>
          )}
          ...
          {order.shipping_address && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 9, letterSpacing: 3, color: T.textDim, fontFamily: T.mono, marginBottom: 8 }}>SHIPPING TO</div>
                ...
                <>
                  {orders.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "48px 0" }}>
                      ...
                      <div style={{ fontSize: 14, color: T.textMid, fontFamily: T.mono, marginBottom: 8 }}>No orders yet</div>
                      <div style={{ fontSize: 12, color: T.textDim, fontFamily: T.mono }}>Complete a checkout to see your orders here.</div>
                    </div>
                  ) : (
                    orders.map((order, index) => {
                      const buildName = order.build_data?.build_name || order.build_name || "Custom Build";
                      const parts = order.build_data?.parts;


                      return (
                        <div key={order.id} style={{ transitionDelay: `${index * 0.07}s` }}>
                          <OrderCard order={order} buildName={buildName} parts={parts} />
                        </div>
                      );
                    })
                  )}
